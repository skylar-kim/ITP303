...
$genreresults = $mysqli->query($genresql);
...
$ratingsresults = $mysqli->query($ratingssql);
if ($ratingsresults == false) {
	echo $mysqli->error;
	exit();
}

$labelssql = "SELECT * FROM labels";
$labelsresults = $mysqli->query($labelssql);
if ($labelsresults == false) {
	echo $mysqli->error; 
	exit();
}

$formatssql = "SELECT * FROM formats";
$formatsresults = $mysqli->query($formatssql);
if ($formatsresults == false) {
	echo $mysqli->error;
	exit();
}

$soundssql = "SELECT * FROM sounds";
$soundsresults = $mysqli->query($soundssql);
if ($soundsresults == false) {
	echo $mysqli->error;
	exit();
}
...
<?php while ($row = $genreresults->fetch_assoc()):?>
...
<?php while ($row = $ratingsresults->fetch_assoc()):?>
...
<?php while ($row = $labelsresults->fetch_assoc()):?>
...
<?php while ($row = $formatsresults->fetch_assoc()):?>
...
<?php while ($row = $soundsresults->fetch_assoc()):?>
